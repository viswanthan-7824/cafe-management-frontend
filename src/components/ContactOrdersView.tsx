      ...
      <div className="glass-card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h2 style={{ fontSize: '1.4rem', fontWeight: 900, color: '#1e293b', display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
            <MessageSquareCheck size={22} color="#4f46e5" /> SAEC CAFÉ Contact & Special Order Request Desk
          </h2>
      ...
      <div className="glass-card" style={{ padding: 0, overflow: 'hidden' }}>
        <table className="custom-table">
          ...
          <tbody>
            {requests.map(req => (
              <tr key={req.id}>
                <td style={{ fontWeight: 900, color: '#4f46e5' }}>{req.request_number}</td>
                <td>
        ...
        <div className="modal-overlay">
          <div className="modal-content animate-fade-in" style={{ maxWidth: '500px' }}>
            ...
            <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', padding: '1rem', borderRadius: '12px', display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.85rem' }}>
              ...
              <div><strong style={{ color: '#64748b' }}>Product:</strong> <span style={{ color: '#4f46e5', fontWeight: 800 }}>{selectedReq.product_name} × {selectedReq.quantity}</span></div>
