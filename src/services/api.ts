...
export const api = {
  ...
  async getOverviewAnalytics(range = 'today'): Promise<AnalyticsOverview | null> {
    try {
      const res = await fetch(`${API_BASE_URL}/analytics/overview/?range=${range}`, { headers: authHeaders() });
      if (!res.ok) return null;
      return await res.json();
    } catch (e) {
      return null;
